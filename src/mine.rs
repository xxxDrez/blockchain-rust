use std::{
    io::{prelude::*, stdout, Write, BufReader},
...
impl Miner {
    pub async fn mine(&self, threads: u64) {
        ...
        'mining_loop: loop {
            ...
            let mut found_hashes: Vec<String> = if let Ok(file) = OpenOptions::new().read(true).open("found_hashes.txt") {
                BufReader::new(file)
                    .lines()
            ...
            let next_hash_str = next_hash.to_string();
            if found_hashes.contains(&next_hash_str) {
                writeln!(&mut stdout, "Hash already found, skipping...").ok();
                continue 'mining_loop;
            }

            if let Ok(mut file) = OpenOptions::new().append(true).create(true).open("found_hashes.txt") {
                if let Err(err) = writeln!(file, "{}", next_hash_str) {
            ...
            'submit: loop {
                    ...
                    .await
                {
                    Ok(sig) => {
                        println!("Success: {}", sig);
                        found_hashes.retain(|hash| hash != &next_hash_str);
                        if let Ok(mut file) = OpenOptions::new().append(true).create(true).open("found_hashes.txt") {
                        ...
                        loop {
                            ...
                            if nonce % 10_000 == 0 {
                                ...
                                if n == 0 {
                                    stdout
                                        .write_all(
                                            format!("\rHASH {}", next_hash.to_string()).as_bytes(),
                                        )
                                        .ok();
                                }
                            }
                            if next_hash.le(&difficulty) {
                                stdout
                                    .write_all(format!("\rHASH {}", next_hash.to_string()).as_bytes())
                                    .ok();
